...
// Fetch harvest data
$harvest_query = "
    SELECT HarvestedDate, AmountOfHarvest 
    FROM harvest 
    WHERE CropId = $crop_id
    ORDER BY HarvestedDate ASC
";
...
const harvest_data = <?php echo json_encode($harvests); ?>;
...
let harvestDates = harvest_data.map(harvest => 
    `${harvest.HarvestedDate}`
).join(", ");

let totalHarvest = harvest_data.reduce((total, harvest) => total + parseFloat(harvest.AmountOfHarvest), 0);
...
let generatedMessage = `I am currently managing a crop of ${data.cropName} (${data.cropType}), 
    ...
    The crop has been harvested on the following dates: ${harvestDates}.
    Amount harvested: ${totalHarvest} kg.
    Based on this information, can you score the performance of the crop on a scale of 1 to 100? 
    The score should reflect overall crop health, weather conditions, pest impact, and harvest yield.
    Yield (20%): This reflects the total harvest from the crop. A higher yield indicates better performance, so this is given significant weight.
Quality (20%): The physical condition of the crop (e.g., size, color, absence of defects). Higher-quality crops get better scores.
Pest Resistance (15%): This evaluates how well the crop resisted pests or diseases. The fewer incidents of pests, the better the score.
Water Efficiency (10%): Efficient use of water resources is crucial, especially in sustainable farming. A crop that uses water efficiently scores higher.
Soil Management (10%): Proper management of soil nutrients and health is important for a healthy crop. Well-maintained soil contributes positively to the score.
Farm Practices (10%): This reflects the implementation of Good Agricultural Practices (GAP). Farms following safe and sustainable methods score higher.
Climate Adaptability (5%): Crops that can thrive in varying climate conditions score higher for their resilience.
Market Demand (5%): Crops that are in high demand or fetch better prices in the market score higher based on market potential.
Harvest Timing (5%): This evaluates whether the crop was harvested at the optimal time, based on planned planting and growth cycles. Timely harvesting improves the score.
    `;
