...
class Login extends DBConnection {
	...
	public function login_nafa() {
		extract($_POST);
		$password = md5($password); // Using MD5 for password encryption
		$stmt = $this->conn->prepare("SELECT * FROM nafa WHERE username = ? and password = ?");
		...
		if ($result->num_rows > 0) {
			...
			$this->settings->set_userdata('login_type', 4); // Set login type to 4 for NAFA users
			return json_encode(array('status' => 'success'));
		} else {
			// Check if the username exists
			$stmt = $this->conn->prepare("SELECT * FROM nafa WHERE username = ?");
			$stmt->bind_param("s", $username);
			$stmt->execute();
			$result = $stmt->get_result();
			
			if ($result->num_rows > 0) {
				return json_encode(array('status' => 'incorrect_password'));
			} else {
				return json_encode(array('status' => 'incorrect_username'));
			}
		}
	}
	
	    public function logout_nafa(){
        if($this->settings->sess_des()){
            redirect('nafa/login.php');
        }
	}
...
switch ($action) {
	case 'login_nafa':
        echo $auth->login_nafa();
        break;
    case 'logout_nafa':
        echo $auth->logout_nafa();
        break;
	case 'login':
		echo $auth->login();
		break;
	...
	case 'logout_client':
		...
		break;
	case 'login_vendor':
		echo $auth->login_vendor();
		break;
	case 'logout_vendor':
		echo $auth->logout_vendor();
		break;
	default:
		echo $auth->index();
		break;
